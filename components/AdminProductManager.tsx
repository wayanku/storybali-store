...
const AdminProductManager: React.FC<AdminProductManagerProps> = ({ products, onUpdateProducts, bannerUrls, onUpdateBanners }) => {
  ...
  const [isAutoSync] = useState(localStorage.getItem('storybali_autosync') === 'true');
  ...
  const handleFetch = async () => {
    if (!scriptUrl) return alert('Atur URL Google Script di menu Sistem!');
    setIsSyncing(true);
  ...
  const handleMultiBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !imgbbKey) return alert('Pilih file & pastikan API Key tersedia');
    setIsUploading(true);
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-32 px-4 md:px-6">
      <div className="bg-white p-5 rounded-3xl shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 border border-stone-100">
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <div>
            <h1 className="text-xl font-bold text-stone-900 tracking-tight">Pusat Penjual</h1>
            <div className="flex items-center gap-2">
              <span className="text-[9px] text-emerald-500 font-black uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded-full">Sistem Aktif</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex bg-stone-50 p-1.5 rounded-2xl shadow-inner flex-1 md:flex-none">
            <button onClick={() => setActiveTab('inventory')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'inventory' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-300'}`}>Stok Barang</button>
            <button onClick={() => setActiveTab('settings')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'settings' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-300'}`}>Sistem</button>
          </div>
          <button onClick={() => { setEditingProduct(null); setFormData({ name: '', price: 0, category: 'Fashion', images: [], description: '', story: '' }); setIsModalOpen(true); }} className="bg-[#ee4d2d] text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-xl hover:scale-105 transition-all">
             <Plus size={16} strokeWidth={3}/> Baru
          </button>
        </div>
      </div>
          ...
          <div className="bg-white p-4 rounded-3xl border border-stone-100 flex flex-col md:flex-row items-center justify-between gap-4">
             <div className="flex items-center gap-3 w-full md:flex-1">
                <div className="relative flex-1">
                   ...
                   <input type="text" placeholder="Cari stok..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-stone-50 border-none rounded-2xl pl-12 pr-6 py-3 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-50 transition-all" />
                </div>
                <div className="flex items-center gap-2">
             ...
             {localProducts.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(p => (
               <div key={p.id} className="bg-white p-4 rounded-[1.8rem] border border-stone-100 shadow-sm flex items-center gap-4 group">
                  ...
                  <div className="flex gap-2">
                     ...
                     <button onClick={() => { if(confirm('Hapus produk?')) { const n = localProducts.filter(x => x.id !== p.id); setLocalProducts(n); handlePush(n); }}} className="p-3 bg-stone-50 text-stone-200 hover:text-red-500 rounded-xl transition-all"><Trash2 size={16} /></button>
                  </div>
               </div>
             ))}
          </div>
        ...
        <div className="space-y-8">
           <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-stone-100 space-y-8">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h3 className="text-xl font-bold text-stone-900 tracking-tight">Banner Promo Beranda</h3>
                    <p className="text-[10px] text-stone-400 font-black uppercase tracking-widest mt-1">Kelola Foto Slider Utama</p>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                 ...
                 <button onClick={() => multiBannerInputRef.current?.click()} disabled={isUploading} className="aspect-[16/9] border-2 border-dashed border-stone-200 rounded-2xl flex flex-col items-center justify-center text-stone-300 hover:border-[#ee4d2d] hover:text-[#ee4d2d] transition-all bg-stone-50">
                    ...
                    <span className="text-[9px] font-black uppercase mt-2 tracking-widest">Tambah Banner</span>
                 </button>
              </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100 space-y-4">
                 <h3 className="text-sm font-black uppercase tracking-widest text-stone-400">URL Sinkronisasi Cloud</h3>
          ...
          <div className="bg-white w-full max-w-6xl md:rounded-[2.5rem] h-full md:h-[90vh] flex flex-col lg:flex-row overflow-hidden animate-in fade-in zoom-in-95">
            <div className="w-full lg:w-96 bg-stone-50 p-6 overflow-y-auto max-h-[30vh] lg:max-h-full border-r border-stone-100">
               <div className="flex justify-between items-center mb-6">
                  <span className="text-[10px] font-black uppercase text-stone-400">Galeri Foto</span>
               ...
               <input type="file" multiple className="hidden" ref={fileInputRef} accept="image/*" onChange={async (e) => {
                  const files = e.target.files;
                  if (!files || files.length === 0 || !imgbbKey) return alert('Data tidak lengkap');
                  setIsUploading(true);
            ...
            <div className="flex-1 p-6 md:p-12 overflow-y-auto bg-white flex flex-col relative">
               <div className="mb-10">
                  <span className="text-[10px] font-black text-[#ee4d2d] uppercase tracking-[0.4em]">Editor Produk</span>
                  <h3 className="text-2xl md:text-4xl font-bold text-stone-900 mt-2">{editingProduct ? 'Perbarui Data' : 'Tambah Produk'}</h3>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                  ...
                  <div className="space-y-3">
                     <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Harga (IDR)</label>
                  ...
                  <div className="col-span-1 md:col-span-2 space-y-3">
                     <div className="flex justify-between items-center px-1">
                        <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Detail Produk</label>
                        <button onClick={async () => {
                             if (!formData.name) return alert('Isi nama produk!');
                             setIsEnhancing(true);
                           ...
                           {isEnhancing ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} Optimalkan AI
                        </button>
                     </div>
                     <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-stone-50 border border-stone-100 rounded-[2rem] p-6 text-sm h-40 resize-none outline-none leading-relaxed" placeholder="Tulis deskripsi produk di sini..."></textarea>
                  </div>
               </div>

               <div className="flex flex-col sm:flex-row gap-4 mt-12 pb-6 md:pb-0">
                  <button onClick={() => setIsModalOpen(false)} className="order-2 sm:order-1 flex-1 py-5 bg-stone-100 rounded-2xl text-xs font-black uppercase tracking-widest text-stone-400">Batal</button>
                  <button onClick={handleSaveLocal} className="order-1 sm:order-2 flex-[2] py-5 bg-stone-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl active:scale-95 transition-all">Simpan Perubahan</button>
               </div>
            </div>
          </div>
        </div>
