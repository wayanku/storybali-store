...
const App: React.FC = () => {
  ...
  const syncFromCloud = useCallback(async (isInitial = false) => {
    ...
    if (cloudData && cloudData.length > 0) {
      // 1. Sinkronisasi Produk (Hanya update jika ada perbedaan)
      const currentProductsJson = JSON.stringify(products);
      // Filter out settings row if it exists in data
      const actualProducts = cloudData.filter(p => p.id !== 'SETTINGS_BANNER');
      
      if (JSON.stringify(actualProducts) !== currentProductsJson) {
        setProducts(actualProducts);
      }

      // 2. Sinkronisasi Banner Global (Mencari row spesial di Sheets)
      ...
      if (bannerSetting && bannerSetting.description) {
        try {
          ...
          console.error("Failed to parse remote banners");
        }
      }
    } else if (isInitial) {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      // Jangan ganggu jika user sedang di halaman admin (biar tidak tumpang tindih saat edit)
      if (route !== AppRoute.ADMIN) {
        syncFromCloud();
      }
    }, 30000);
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
           <div className="relative h-[220px] md:h-[400px] rounded-3xl overflow-hidden group shadow-xl">
              ...
              {heroBanners.length > 1 && (
                <>
                  <button onClick={() => setActiveBannerIdx(prev => (prev - 1 + heroBanners.length) % heroBanners.length)} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 bg-white/20 backdrop-blur-md text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-stone-900"><ChevronLeft size={24} /></button>
                  <button onClick={() => setActiveBannerIdx(prev => (prev + 1) % heroBanners.length)} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 bg-white/20 backdrop-blur-md text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-stone-900"><ChevronRight size={24} /></button>
              ...
              <div className="absolute bottom-10 left-6 md:bottom-16 md:left-12 text-white space-y-2 md:space-y-4 z-20 pointer-events-none">
                 ...
                 <h2 className="text-3xl md:text-6xl font-black leading-tight drop-shadow-lg">StoryStore Global</h2>
                 <p className="text-xs md:text-lg opacity-90 font-bold max-w-lg drop-shadow-md">Koleksi pilihan terbaik dari seluruh dunia, kini dalam jangkauan Anda.</p>
              </div>
           </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-2 md:px-4">
         <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden border border-stone-50">
            <div className="p-5 md:p-8 flex items-center justify-between">
               ...
               <button onClick={() => setRoute(AppRoute.CATALOG)} className="text-[#ee4d2d] text-xs font-black uppercase tracking-widest flex items-center gap-2">See All <ArrowRight size={14} /></button>
            </div>
               ...
               {products.slice(0, 6).map(p => (
                 <div key={p.id} onClick={() => handleViewDetail(p)} className="min-w-[160px] md:min-w-[200px] cursor-pointer group">
                    <div className="relative aspect-square rounded-2xl overflow-hidden bg-stone-50 mb-4 shadow-inner">
                       ...
                       <div className="absolute top-3 right-3 bg-[#ee4d2d] text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg">-25%</div>
                    </div>
                    <div className="space-y-3">
                       <p className="text-stone-900 font-black text-sm md:text-base">Rp {(p.price * 0.75).toLocaleString('id-ID')}</p>
                       <div className="relative h-4 bg-stone-100 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-orange-400 to-[#ee4d2d]" style={{width: '70%'}}></div>
                       </div>
                    </div>
                 </div>
               ))}
               ...
               <div className="max-w-md mx-auto py-24 px-4">
                 <div className="bg-white p-12 rounded-[3rem] shadow-2xl space-y-10 border border-stone-100 text-center">
                   ...
                   <div className="space-y-2">
                     ...
                     <p className="text-[10px] text-stone-300 font-black uppercase tracking-[0.2em]">Restricted Area</p>
                   </div>
               ...
               <div className="max-w-4xl mx-auto py-10 px-4">
                  <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-sm space-y-10 border border-stone-100">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2"><label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Penerima</label><input type="text" value={address.name} onChange={e => setAddress({...address, name: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-5 rounded-xl outline-none font-bold" placeholder="Nama Lengkap" /></div>
                        <div className="space-y-2"><label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">WhatsApp</label><input type="text" value={address.phone} onChange={e => setAddress({...address, phone: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-5 rounded-xl outline-none font-bold" placeholder="62812..." /></div>
                        <div className="col-span-1 md:col-span-2 space-y-2"><label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Alamat</label><textarea value={address.detail} onChange={e => setAddress({...address, detail: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-6 rounded-xl h-32 outline-none resize-none" placeholder="Detail alamat lengkap..."></textarea></div>
                     </div>
                     <div className="bg-stone-900 p-8 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl">
                        <div><p className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Grand Total</p><p className="text-3xl font-black text-[#ee4d2d] mt-1 italic">Rp {cartTotal.toLocaleString('id-ID')}</p></div>
                        <button onClick={() => { if (!address.name || !address.phone || !address.detail) return alert('Lengkapi data pengiriman!'); const itemsStr = cart.map(item => `- ${item.name} (x${item.quantity})`).join('%0A'); const msg = `Halo StoryStore!%0A%0ASaya ingin memesan:%0A${itemsStr}%0A%0ATotal: Rp ${cartTotal.toLocaleString('id-ID')}%0A%0APenerima: ${address.name}%0AAlamat: ${address.detail}`; window.open(`https://wa.me/6281234567890?text=${msg}`); setCart([]); setRoute(AppRoute.HOME); }} className="w-full md:w-auto px-12 py-5 bg-[#ee4d2d] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-3">Order on WhatsApp <ArrowRight size={18} /></button>
                     </div>
                  </div>
               </div>
            )}
