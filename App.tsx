...
const App: React.FC = () => {
  ...
  // Dynamic Banners State (Array of strings)
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
           <div className="relative h-[220px] md:h-[400px] rounded-3xl overflow-hidden group shadow-xl">
              ...
              {heroBanners.length > 1 && (
                <>
                  <button 
                    ...
                    className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 bg-white/20 backdrop-blur-md text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-stone-900"
                  >
                    <ChevronLeft size={24} />
                  </button>
                  <button 
                    onClick={() => setActiveBannerIdx(prev => (prev + 1) % heroBanners.length)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 bg-white/20 backdrop-blur-md text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-stone-900"
                  >
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                    {heroBanners.map((_, idx) => (
                      <button 
                        ...
                        className={`h-1.5 rounded-full transition-all duration-500 ${idx === activeBannerIdx ? 'w-8 bg-[#ee4d2d]' : 'w-2 bg-white/50'}`}
                      />
                    ))}
                  </div>
                </>
              ...
              <div className="absolute bottom-10 left-6 md:bottom-16 md:left-12 text-white space-y-2 md:space-y-4 z-20 pointer-events-none">
                 ...
                 <h2 className="text-3xl md:text-6xl font-black leading-tight drop-shadow-lg">Global Mega Sale</h2>
                 <p className="text-xs md:text-lg opacity-90 font-bold max-w-lg drop-shadow-md">Eksplorasi koleksi eksklusif dari brand internasional favorit Anda di StoryStore.</p>
              </div>
           </div>
        </div>
      </section>
                 ...
                 <div key={p.id} onClick={() => handleViewDetail(p)} className="min-w-[160px] md:min-w-[200px] cursor-pointer group">
                    ...
                    <div className="space-y-3 px-1 text-center md:text-left">
                       ...
                       <div className="relative h-5 bg-stone-100 rounded-full overflow-hidden border border-stone-50">
                          <div className="absolute inset-0 flex items-center justify-center text-[9px] font-black text-white mix-blend-difference z-10 uppercase tracking-tighter">Limited Stock</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16">
          ...
          <div className="flex flex-col h-full">
            <div className="space-y-6">
              ...
              <div className="space-y-8 py-4">
                <div className="space-y-3">
                   <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-[0.3em]">Deskripsi</h3>
            ...
            <div className="mt-auto pt-10 grid grid-cols-1 sm:grid-cols-2 gap-4">
               <button 
                 onClick={() => addToCart(p)}
                 className="w-full bg-stone-900 text-white py-5 md:py-6 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-stone-800 transition-all"
               >
                 Add to Cart
               </button>
               <button 
                 onClick={() => { addToCart(p); setRoute(AppRoute.CART); }}
                 className="w-full bg-[#ee4d2d] text-white py-5 md:py-6 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all"
               >
                 Buy Now
               </button>
            </div>
      ...
      <main className="main-content">
        {isLoading ? (
          <div className="h-[70vh] flex flex-col items-center justify-center gap-4">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-300">Loading StoryStore...</p>
          </div>
        ) : (
          <>
              ...
              <div className="py-6 md:py-10">
               ...
               <div className="max-w-md mx-auto py-24 px-4">
                 <div className="bg-white p-12 rounded-[3rem] shadow-2xl space-y-10 border border-stone-100 text-center">
                   ...
                   <div className="space-y-2">
                     <h2 className="text-2xl font-bold text-stone-900 tracking-tight">Seller Center</h2>
                     <p className="text-[10px] text-stone-300 font-black uppercase tracking-[0.2em]">Restricted Access</p>
                   </div>
                   <div className="space-y-4">
                     <input 
                       ...
                       className="w-full bg-stone-50 border border-stone-100 p-6 rounded-2xl outline-none text-center font-black text-3xl tracking-[0.5em] text-stone-900"
                     />
                     <button onClick={handleAdminLogin} className="w-full bg-[#ee4d2d] text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all">Unlock System</button>
                   </div>
                 </div>
               </div>
            ))}
            {route === AppRoute.CHECKOUT && (
               <div className="max-w-4xl mx-auto py-10 px-4">
                  <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-sm space-y-10 border border-stone-100">
                     <h1 className="text-2xl font-bold flex items-center gap-3 border-b border-stone-50 pb-8"><MapPin className="text-[#ee4d2d]"/> Shipping Details</h1>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Penerima</label>
                           <input type="text" value={address.name} onChange={e => setAddress({...address, name: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-5 rounded-xl outline-none font-bold" placeholder="Nama Lengkap" />
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">WhatsApp</label>
                           <input type="text" value={address.phone} onChange={e => setAddress({...address, phone: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-5 rounded-xl outline-none font-bold" placeholder="62812..." />
                        </div>
                        <div className="col-span-1 md:col-span-2 space-y-2">
                           <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Alamat</label>
                           <textarea value={address.detail} onChange={e => setAddress({...address, detail: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-6 rounded-xl h-32 outline-none resize-none" placeholder="Detail alamat lengkap..."></textarea>
                        </div>
                     </div>
                     <div className="bg-stone-900 p-8 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl">
                        <div>
                           <p className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Grand Total</p>
                        ...
                        <button 
                          onClick={() => {
                            ...
                            const msg = `Halo StoryStore!%0A%0ASaya ingin memesan:%0A${itemsStr}%0A%0ATotal: Rp ${cartTotal.toLocaleString('id-ID')}%0A%0APenerima: ${address.name}%0AAlamat: ${address.detail}`;
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 pb-24">
       <div className="relative w-full md:w-[500px] mb-12">
          ...
          <input 
            ...
            className="w-full bg-white border border-stone-100 rounded-[2rem] pl-16 pr-8 py-5 text-sm font-bold shadow-sm outline-none"
          />
       </div>
       <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-6">
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 pb-32">
       <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-sm border border-stone-100 mb-8 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-stone-900 tracking-tight">Shopping Cart ({cart.length})</h1>
          <button onClick={onBack} className="text-xs font-black uppercase tracking-widest text-stone-400 hover:text-[#ee4d2d]">Back to Store</button>
       </div>

       {cart.length === 0 ? (
          <div className="bg-white p-24 text-center rounded-[3rem]">
             <ShoppingBag size={80} className="mx-auto text-stone-100 mb-8" />
             <p className="text-stone-400 font-bold mb-10">Keranjang Anda kosong.</p>
             <button onClick={onBack} className="bg-stone-900 text-white px-12 py-4 rounded-2xl font-black text-xs uppercase tracking-widest">Start Shopping</button>
          </div>
       ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             ...
             <div className="lg:col-span-1">
                <div className="bg-stone-950 p-8 rounded-[2.5rem] shadow-2xl text-white space-y-8">
                   ...
                   <button onClick={onCheckout} className="w-full bg-[#ee4d2d] text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Checkout</button>
                </div>
             </div>
          </div>
       )}
